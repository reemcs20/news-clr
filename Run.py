...
if __name__ == '__main__':
    """determine if the search operation is arabic or english. some search engines are for AR or the opposite ir BOTH.
    """
    if language == 'ar':
        "Perform a AR language news gathering"
        ...
        sky_news_process = multiprocessing.Process(target=sky_news.RunExtraction, args=(language,))
        ...
        "Perform a EN language news gathering"
        aljazeera = Aljazeera(query=query.encode('utf-8'), language=language)
        ...
        rt_process.start()
        # Join Processes which means wait until the processes end then continue execution
        aljazeera_process.join()
        alarabiya_process.join()
